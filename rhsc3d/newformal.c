...
double Formal(int nspect, bool_t eval_operator, bool_t redistribute)
{
  ...
  int      Nspace = atmos.Nspace, Nrays = atmos.Nrays, la, Nrecphi;
  double  *phi, *I, *chi, *S, **Ipol, **Spol, *Psi, *Jdag, wmu, dJmax, dJ,
          *eta_Q, *eta_U, *eta_V, *eta_c_Q, *eta_c_U, *eta_c_V,
    *J20dag, musq, threemu1, threemu2, *J, *J20, *reJ21, *imJ21, *reJ22, *imJ22, inc, azi, wlambda;
  ActiveSet *as;
  AtomicLine *line;
  Atom *atom;
  ...
  for (nact = 0;  nact < atmos.Nactiveatom;  nact++) {
	...
	j = line->j;
	n_i = atom->n[i];
	n_j = atom->n[j];  I might not need these.
	*/
  ...
  if (angle_dep) {
	...
	azi   = atan(geometry.muy[mu]/geometry.mux[mu]);
	...
	if (eval_operator) {
	  ...
	  if (input.backgr_pol) {
	    ...
	    reJ21[k] += (sqrt(3.0)/2.0)*( sqrt(1.0 - musq) * ( -1.0 * mu * cos(azi) * (Ipol[0][k] + Ipol[1][k]) + sin(azi) * Ipol[2][k]) )  * wmu;

	    imJ21[k] += (sqrt(3.0)/2.0)*( sqrt(1.0 - musq) * ( -1.0 * mu * sin(azi) * (Ipol[0][k] + Ipol[1][k]) - cos(azi) * Ipol[2][k]) ) * wmu;


	    reJ22[k] += (sqrt(3.0)/4.0)*(cos(2.0*azi)*((1.0-musq) * Ipol[0][k] - (1.0 + musq)* Ipol[1][k]) + 2.0*sin(2.0*azi)*inc*Ipol[3][k]) * wmu;

	    imJ22[k] += (sqrt(3.0)/4.0)*(sin(2.0*azi)*((1.0-musq) * Ipol[0][k] - (1.0 + musq)* Ipol[1][k]) - 2.0*cos(2.0*azi)*inc*Ipol[3][k]) * wmu;
	    }
	  }
