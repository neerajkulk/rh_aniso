...
/*
  This is the function that calculates the spherical tensors.
  It uses the short charactersitcs from the formal solver to calculate the intensity field.
  From the intensity field, spherical tensors are calculated by integrating the intensity
  ...
  */
  ...
  for (nspect = 0; nspect <= spectrum.Nspect - 1; nspect++)
  {
    ...
    for (nact = 0; nact < atmos.Nactiveatom; nact++)
    {
      ...
      for (n = 0; n < as->Nactiveatomrt[nact]; n++)
      {
        ...
        if (fabs(line->lambda0 - 460.861732) < 0.00001) // Hardcoded central wavelength of strontium.
        {
  ...
  for (nspect = ni; nspect <= nf; nspect++)
  { // For loop over SR wavelengths
    as = &spectrum.as[nspect];
    alloc_as(nspect, eval_operator);

    // Do a formal solution for each wavelength in the line
    ...
    if (angle_dep)
    {
      ...
      for (mu = 0; mu < Nrays; mu++)
      {
        ...
        // Now intensities are calculated. We need to perform spherical integration. 
        ...
        if (atmos.moving)
        {
          for (to_obs = 0; to_obs <= 1; to_obs++)
          {
            ...
            for (k = 0; k < atmos.Nspace; k++)
            {
              ...
              domg_dlam = wmu * line->phi[lamu][k] * line->wphi[k] * wlambda; //this is the differential element in wavelength and angle
        ...
        {
          // stationary atmos (No velocities)
          for (k = 0; k < atmos.Nspace; k++)
          {

            domg_dlam = 2.0 * wmu * line->phi[la][k] * line->wphi[k] * wlambda;
            // need to multiply by 2 if vel = 0 since we account for only half of the spectral line. 
...
// write spherical tensors and other data to .txt files
