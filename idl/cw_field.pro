; $Id: //depot/Release/ENVI53_IDL85/idl/idldir/lib/cw_field.pro#1 $
;
...
;   CT, VIS, Aug 2013: Add DOUBLE, ULONG, and FOCUS_EVENTS keywords.
;         Allow IGNORE_ACCELERATORS to pass through.
;
;-
...
FUNCTION CW_FIELD_VALUE, Value, Type
    ...
    3: begin
      tmp = LONG64(NValue)
      RETURN, (ABS(tmp) gt 2147483647) ? tmp : LONG(NValue)
      end
    4: begin
      tmp = ULONG64(NValue)
      RETURN, (tmp gt 4294967295) ? tmp : ULONG(NValue)
      end
    5:  RETURN, DOUBLE(NValue)
    ENDCASE
END
...
;
PRO CW_FIELD_INT, Ch, State, Event, Altered, Type
    ...
    IF Ch EQ 45b THEN begin
      if (type eq 4) then return ; unsigned int, minus sign not allowed
      Negate = 1
    endif ELSE IF Ch GE 48b AND Ch LE 57b THEN BEGIN
      Nil = 0 & Minus = 0
    ENDIF ELSE begin
      RETURN   ; ![0-9]
    ENDELSE
...
;
PRO CW_FIELD_FLOAT, Ch, State, Event, Altered, Type
...
FUNCTION CW_FIELD_EVENT, Event
  ...
  if (TAG_NAMES(Event, /STRUCTURE_NAME) eq 'WIDGET_KBRD_FOCUS') then begin
    if (event.enter eq 1) then return, 0
    ; Create a widget text event with a carriage return.
    oldevent = event
    event = {WIDGET_TEXT_CH}
    event.id = oldevent.id
    event.top = oldevent.top
    event.handler = oldevent.handler
    event.type = 0
    event.offset = 0
    event.ch = 10b
  endif
  ...
  IF State.Type NE 0 THEN BEGIN       ; Not a String?
    ...
    IF Event.Type LE 1 THEN BEGIN       ; Insert Characters?
      ...
      Procedure = (State.Type EQ 1 || State.Type eq 5) ? $
        'CW_FIELD_FLOAT' : 'CW_FIELD_INT'
  
      Altered = 0
      IF Event.Type EQ 0 THEN BEGIN
        CALL_PROCEDURE, Procedure, Event.Ch, State, Event, Altered, State.Type
      ENDIF ELSE BEGIN
        Chars   = BYTE(Event.Str)
        FOR I=0,N_ELEMENTS(Chars)-1 DO $
          CALL_PROCEDURE, Procedure, Chars[I], State, Event, Altered, State.Type
      ENDELSE
