; $Id: //depot/Release/ENVI53_IDL85/idl/idldir/lib/write_csv.pro#2 $
;
; Copyright (c) 2008-2015, Exelis Visual Information Solutions, Inc. All
...
;   CT, Oct 2015, IDL-69458: Properly escape quotes and commas in headers.
;   
;-
...
; What about handling COMMAS or QUOTES?!
  ...
  if n_elements(tableHeader) gt 0 then begin
    foreach h, tableHeader do begin
      if (h.Contains(',') || h.Contains('"')) then begin
        h = (h.Split('"')).Join('""')
        h = '"' + h + '"'
      endif
      printf, lun, h
  ...
  if (nheader gt 0) then begin
    ...
    endforeach
    PRINTF, lun, header1, FORMAT=format
  endif
